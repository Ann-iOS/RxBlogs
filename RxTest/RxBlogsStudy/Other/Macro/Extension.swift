...
//        self.extSetBorderWidth(1, color: UIColor.green)
    ...
    */
    ...
    /**
     弹出模态视图控制器
     UIColor.init(red: red/255.0, green: green/255.0, blue: blue/255.0, alpha: alpha)
     - parameter vc:              控制器
     - parameter backgroundColor: 背景色 可不传 有默认值
     - parameter animated:        是否动画 可不传 有默认值 默认无动画
     */
    public final func extShowModalVC(_ vc :UIViewController , backgroundColor :UIColor = UIColor.init(red: 66/255, green: 66/255, blue: 66/255, alpha: 0.4) , animated :Bool = false){
